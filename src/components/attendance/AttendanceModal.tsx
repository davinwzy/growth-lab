...
export function AttendanceModal({ isOpen, onClose }: AttendanceModalProps) {
  ...
  const dateAttendance = useMemo(() => {
    return classAttendance.filter(r => r.date === selectedDate);
  ...
  const calendarDays = useMemo(() => {
    ...
    const days: { date: string; day: number; isCurrentMonth: boolean; attendance: number; total: number }[] = [];
    ...
    for (let i = startPadding - 1; i >= 0; i--) {
      ...
      days.push({ date: dateStr, day: d.getDate(), isCurrentMonth: false, attendance: dayAttendance, total: currentStudents.length });
    }
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      const dayAttendance = classAttendance.filter(r => r.date === dateStr && r.status === 'present').length;
      days.push({ date: dateStr, day: d, isCurrentMonth: true, attendance: dayAttendance, total: currentStudents.length });
    }
    ...
    for (let d = 1; d <= remaining; d++) {
      ...
      const dayAttendance = classAttendance.filter(r => r.date === dateStr && r.status === 'present').length;
      days.push({ date: dateStr, day: d, isCurrentMonth: false, attendance: dayAttendance, total: currentStudents.length });
    }

    return days;
  }, [calendarMonth, classAttendance, currentStudents.length]);
  ...
  const handleSelectAll = () => {
    const notMarked = currentStudents.filter(s => !attendedOnDate.has(s.id)).map(s => s.id);
  ...
  const handleSubmit = async () => {
    ...
    for (const studentId of presentIds) {
      ...
      if (student) {
        ...
        if (isToday) {
          processAttendance(studentId, student.name);
        } else {
          processAttendanceMakeup(studentId, student.name, selectedDate);
        }
      }
    }
  ...
  const handleRevokeAttendance = (studentId: string) => {
    ...
    const record = classAttendance.find(r => r.studentId === studentId && r.date === selectedDate && r.status === 'present');
    if (record) {
    ...
    revokeAttendance(studentId, student.name, selectedDate);
  };
