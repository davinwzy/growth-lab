...
type FilterType = 'all' | 'score' | 'reward';
type FilterTarget = 'all' | 'student' | 'group';
...
export function HistoryList({ isOpen, onClose }: HistoryListProps) {
  ...
  const handleExport = () => {
    const csv = [
      ['时间', '类型', '目标', '对象', '项目', '分数', '备注', '已撤销'].join(','),
      ...filteredHistory.map(h => [
        new Date(h.timestamp).toLocaleString(),
        h.type === 'score' ? '加减分' : '兑换',
        h.targetType === 'student' ? '学生' : '组别',
        h.targetName,
        h.itemName,
        h.value,
        h.note || '',
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs px-1.5 py-0.5 bg-white/50 rounded">
                        {record.targetType === 'student' ? t('学生', 'Student') : t('组别', 'Group')}
                      </span>
                      {record.undone && (
