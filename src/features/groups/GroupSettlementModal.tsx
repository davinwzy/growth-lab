...
import type { HistoryRecord, GamificationSnapshot } from '@/shared/types';
import { cloneGamificationSnapshot } from '@/shared/utils/gamification';
import { computeSettlementBonuses, groupStudentsByGroupId, sortGroupsByScore } from '@/services/groupSettlement';
...
export function GroupSettlementModal({ isOpen, onClose }: GroupSettlementModalProps) {
  ...
  const currentGroups = useMemo(() => {
    return sortGroupsByScore(state.groups.filter(g => g.classId === state.currentClassId));
  ...
  const handleSettle = () => {
    ...
    const bonusPayload = computeSettlementBonuses(currentGroups, bonuses);

    // Process gamification for each student
    currentGroups.forEach((group, index) => {
      const bonus = getBonusForRank(index);
      const groupStudents = groupStudentsByGroupId(state.students, group.id);
      ...
      const record: HistoryRecord = {
        ...
        perStudentDeltas,
        groupScoreBefore: group.score || 0,
      };
